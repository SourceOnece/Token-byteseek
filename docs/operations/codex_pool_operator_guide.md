...
> 逻辑基线为应用 **bh.051**；批量表单按 **bh.053** 的收敛要求更新，核对日期：2026-09-20（Asia/Taipei）。发布状态见[bh.053记录](versions/v0_1_278_bh_053.md)，此前流程图整理见[bh.052记录](versions/v0_1_278_bh_052.md)。本文没有替你开启任何配置。
>
...
bh.051 起，批量编辑读取所选账号：值都一样就显示共同值，例如两个号都是332就显示332；不一样就留空。bh.053按用户要求撤掉“更多账号配置”，保留原有批量项目和票据工作台。票据项与原Base URL一样：右侧勾选，未选虚化且不可编辑，勾选后实化；只有勾选项提交。新号默认模板、已有账号重导入不覆盖票据的规则不变。
...
本手册最初在[bh.048](versions/v0_1_278_bh_048.md)建立，[bh.052](versions/v0_1_278_bh_052.md)补齐流程图，[bh.053](versions/v0_1_278_bh_053.md)收敛批量界面；采集/调度逻辑未因此改变。应用镜像和是否上线以实际发布、部署证据为准；后续实现变化时，应一起更新本手册。
