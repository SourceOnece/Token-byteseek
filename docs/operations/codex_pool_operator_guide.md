...
> 按 ByteSeek **bh.047** 的代码编写，核对日期：2026-09-19（Asia/Taipei）。应用源码：`54bd681a6c649721090e0465dfc811059d2b802f`。不是照搬上游说明，也不代表已经替你开启了配置。
>
...
| 账号采集并发 | 同号采集任务1–4，外层还有全局4worker | 1号有2个模型，设1会串行争用槽；全站也不会因此变成无限采集 |
...
### 页面里为什么还有“不改账号开关”的旧说明？

当前bh.047的部分短文案仍残留旧语义，例如批量采集的资格提示提到“不改账号开关”。**按当前实际代码，合格采集会开、明确异常长度会关。** 本次仅写手册，没有修改这段UI文案；不要把旧提示当成“只观察”的保证。
...
本次文档编号为[bh.048](versions/v0_1_278_bh_048.md)，**应用镜像仍是bh.047**；纯文档不重新构建镜像、不修改latest/bauhaus、不替换线上容器。后续实现变化时，应一起更新本手册。
