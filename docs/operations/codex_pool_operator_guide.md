...
> 对应已发布应用 **bh.051**，源码 `63768bb06`，核对日期：2026-09-20（Asia/Taipei）。本轮文档编号是 **bh.052**，应用镜像没有变。实现与验证见[bh.051记录](versions/v0_1_278_bh_051.md)，本次说明整理见[bh.052记录](versions/v0_1_278_bh_052.md)。本文没有替你开启任何配置。
>
...
| 在哪里改 | 按什么保存 | 改谁 |
| --- | --- | --- |
| 导入默认值里的票据工作台 | 工作台自己的“保存票据配置” | 以后的新号模板 |
| 添加OpenAI OAuth账号 | 随本次账号创建/导入提交 | 本次实际新建的账号 |
| 已有账号编辑里的票据工作台 | “保存票据配置” | 这个账号的票据规则 |
| 批量编辑里的票据工作台 | 勾选项目，再“保存票据配置” | 所选支持账号的勾选项目 |

默认值页面还有其它普通设置的“保存”，它不代替票据工作台自己的保存。已有账号中的业务代理等普通设置也要保存原账号表单。**票据设置保存成功，不表示另一个区域的草稿也保存了。**
...
入口：**账号管理→勾选账号→批量编辑选中账号**。本次新增的完整配置区和票据工作台针对这个“勾选账号”入口；“按筛选条件批量”仍是原有入口，不假装已经逐一读取整个筛选结果。
...
| 分别是292和332 | 空白 | 勾选长度，填356，再保存票据 | 两个号都改为356 |
| 双链路一个开、一个关 | 空白 | 勾选后明确选关闭 | 两个号都关闭双链路 |
| 备注相同 | 共同备注 | 勾选备注，再勾“清空”，批量更新 | 所选账号备注清空 |
| 密码相同或不同 | 始终不回显 | 不勾选、不填新密码 | 保留原密码 |

**这里的空白是“无法显示一个共同值”，不是0、不是关闭、也不是已经清空。** 0在某些字段有明确意义，例如票据最多尝试0表示不限次数，不能把混合值空白自动当作0。

### 19.2 批量有哪些配置

页面根据所选账号的平台和类型显示适用字段。不同平台混选时，仅共同适用的部分能够一起设置。下表按用途整理，具体数值语义沿原新增账号配置；不会因为放到批量页面就变成全站设置。

| 配置类别 | 包含什么 | 通俗作用 |
| --- | --- | --- |
| 基础管理 | 名称、备注、到期、到期暂停、启用状态、总调度 | 给这一批号改管理信息和接单资格 |
...
| 额度管理 | 总/每日/每周额度、重置模式、时间和时区、通知开关/阈值 | 管理账号本地限额和通知；不改变上游官方限额 |
| 错误和流量策略 | 池模式、同号重试、错误码、临时停调规则、预热、请求头、RPM/消息排队 | 调整原有失败或流量处理策略；不要随意给不懂的规则填空对象 |
| Anthropic相关 | 透传/鉴权、联网搜索兼容、窗口费用、会话、缓存时长、自定义地址 | 使用该平台原有规则，只有适用账号才显示 |
| Gemini/Vertex | 项目、等级、授权类型、来源、区域、服务账号JSON/邮箱 | 连接指定项目和云环境；改成错误项目会影响实际可用性 |
| Antigravity | 项目、混合调度、允许超额 | 调整原平台配置，保留原混合渠道校验 |
| 国产平台/OpenCode | 套餐模式、协议、多协议地址、模型协议规则、智谱组织/项目 | 指定实际上游套餐和协议；不是自动同时测试所有协议 |
| Bedrock/Qoder及密钥 | AWS区域/鉴权/密钥，Qoder站点/身份字段/令牌，API Key和钱包查询信息 | 对所选账号显式更新凭据字段，旧秘密不回显 |
| 票据工作台 | 开关、模型、两种长度、次数、并发、续期、冷却、守护、双链路、采集代理 | 这一块仍是独立保存；规则见第8至13节 |

OAuth登录、授权码交换、导入动作，以及切换账号平台/类型，不是“把某个表单值批量改一下”；原身份建立流程仍保留。普通编辑中的一些复杂规则保留JSON输入，数组和对象类型必须正确。

### 19.3 为什么要勾选、为什么分开保存

假设100个号的并发本来不同，你只想统一合格长度。不勾选并发就不会发送并发字段，避免把界面某个默认数字刷到全部账号。秘密字段也一样，留空不表示让所有账号没有密码。

票据批量接口先校验整个选择集合和本次规则，再一次保存私有配置；一个号不支持或规则无效，本次票据保存整体拒绝。普通账号批量更新沿原接口，分组等步骤可能返回逐号部分成功，因此要看结果提示；**普通批量更新和独立票据保存不是一个跨两个接口的总事务**。要同时改业务代理和票据，需要分别保存，并分别确认结果。
...
当前已发布应用及准确digest见[bh.051](versions/v0_1_278_bh_051.md)。本文不会因写说明再打一个内容相同的新镜像，也没有验证你另一台服务器现在实际运行的是哪版。
...
| 普通批量配置与相同值回填 | [批量表单](../../frontend/src/components/account/BulkEditAccountModal.vue)、[扩展字段](../../frontend/src/components/account/bulkAdditionalFields.ts)、[字段编辑器](../../frontend/src/components/account/AccountBulkAdditionalSettings.vue) |
...
本手册最初在[bh.048](versions/v0_1_278_bh_048.md)作为纯文档建立，之后随bh.049和bh.051更新；当前流程图整理记录为[bh.052](versions/v0_1_278_bh_052.md)，对应应用仍是bh.051。应用镜像和是否上线以实际发布、部署证据为准；后续实现变化时，应一起更新本手册。
