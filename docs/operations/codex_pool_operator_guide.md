...
> 采集判定沿原逻辑；**bh.057**收拢新号模板、账号运行配置和历史兼容，重排导入默认值已有字段；保留成功IP复用和工作台统一保存。核对日期：2026-09-20（Asia/Taipei），发布状态见[bh.057记录](versions/v0_1_278_bh_057.md)。本文没有替你开启任何配置。
>
...
| 单号超时（秒） `timeout_seconds` | 120；10–3600；旧API省略或0按120 | 每个号本次最多等多久；慢思考可自行提高，但不影响票据工作台的独立超时 |
...
**没有单独记录的旧账号不是天然“关闭票据”。** bh.057保留它升级前的实际开关与规则快照；总闸开了后，符合条件且未设off的旧号仍可能进入采集/门控。新号则以导入默认值初始化。双链路初始默认仍关闭。
...
| 票据开关 `mode` | 明确on/off；旧inherit在读取边界还原为原来的on | off不采、不注入、不按缺票拦这个账号；**不等于关掉整个账号业务** |
...
- 新增OpenAI账号时，票据只在第一步“授权方式”配置。第二步“OpenAI账户授权”专心填授权资料，不重复显示工作台；返回第一步可继续改，已填配置保留并随最终创建提交。
- 单号票据表单加载实际规则；没有独立记录的旧号保留升级前的冻结快照，不再跟随可修改的旧网关规则。不能拿下面的“代码缺省值”覆盖你自己的值。
...
合格长度输入值为加粗绿色，异常长度信号输入值为加粗红色，方便区分。异常信号填0仍表示关闭该信号，红色不是账号检测结论；批量未勾选时仍虚化、不会修改。
...
| 单次采集超时（秒） `attempt_timeout_seconds` | 25；5–300整数 | 取代理、候选采集和业务复验共用。例如填120，两步合计最多120秒，不是各120秒；排队和收尾另有短预算 |
...
| 异常守护 `watchdog_mode` | 新模板缺省observe；旧继承先保留原有效策略 | 业务响应异常如何观察/废票；选项细节见第12节 |
...
flowchart TD
    ...
    B -->|"有"| C["使用账号采集配置"]
    B -->|"没有"| D["使用网关默认采集配置"]
    C --> R{"轮换/动态已开启复用，且有本号本模型可用的成功代理记录？"}
    D --> R
    R -->|"有"| S["复用成功代理和原会话，不再取号"]
    R -->|"没有"| E{"有效代理模式"}
    E -->|"固定"| F["使用指定的一条"]
    ...
    S --> M
    M --> N{"最终成功存票？"}
    N -->|"是，且已开启复用"| O["记住本次成功采集代理，下一次优先使用"]
    N -->|"不合格且允许重试"| P["弃用失败的复用记录，按原次数/间隔/冷却换新"]
```
...
| 动态代理→地址模板 | 填支持会话标识的完整代理URL | 默认每次生成新的`{sid}`/`{random}`，只替换认证信息；开启复用且已有成功记录则继续用原SID |
| 动态代理→接口取号 | 填HTTPS取号API，再选返回代理协议 | 默认每次实际采集前GET接口取一条；开启复用且已有成功记录时不重复取号。复验仍走业务代理 |

### 10.2.1 复用合格IP：新开关怎么用

- 网关默认：设置→网关服务→OpenAI→票据采集→勾选修改默认采集代理。
- 单号覆盖：账号编辑→票据采集工作台→勾选修改采集代理。
- 代理模式选“轮换”或“动态”，才会出现“复用合格IP”；默认关闭，固定模式没有这个开关。
- 账号选择“继承网关代理”时一起继承网关的复用设置。选专用代理则按账号设置，网关开不代表专用账号也开。
- 批量仍先勾选工作台主开关并开启，再勾选修改采集代理；复用开关随这份代理配置和原批量按钮保存。新号默认模板里也使用同一编辑器。

打开后：**成功获票→记住这次采集代理→下次优先用它；复用时不合格→弃用这条成功记忆→允许重试时再换。** 双链路必须两阶段通过并成功存票，才会记住候选采集代理；不会把固定业务代理替换成这个采集代理。

“复用IP”表示走上次成功记录，“新IP”表示本次重新选择/生成或取号。标签可在模型鼠标悬停、点击详情、采集日志中看见；它不保证真实公网IP一定相同/不同，服务商可能自动更换或重复分配。参考IP仍是独立查询，不是抓包证明。

成功记忆按账号、模型、凭据及配置分开，加密保存在共享Redis，重启应用后仍可用；24小时没有成功采集就过期，每次成功续期。改配置/凭据后旧记录不会混入新规则，管理代理改地址、停用或过期也不会继续使用旧地址。缓存读故障会停止本次采集；记忆写失败不会撤销已保存的合格票，但下次可能重新取号。

401/403/429、认证/额度拒绝、Retry-After、次数与冷却仍按原规则处理，**不会通过不停换IP绕过限制**。取消/存储错误也不会直接把代理判成不合格。“测试代理”按钮只测连接出口，不会建立成功获票记忆。
...
| 字段 | 怎么理解 |
| --- | --- |
| 动态来源→接口取号 | 表示向供应商拿代理，不是把这个API当模型服务；命中成功复用记录时不重复取号 |
...
两张详情卡的字节数都是**各自响应中返回的STATE长度**，不是发出去的票据长度。候选采集合格绿、命中异常信号红、其它黄；业务复验命中异常信号红、0或合格绿、其它黄，信号优先。复验`HTTP 200 · 0 B`通常表示已带候选票请求，但上游没返回新STATE；完整响应/模型等校验仍要通过，0绿并不把失败请求判为成功。成功后存的仍是第一张候选票，不是空票。
...
bh.057移除守护“继承网关”选项；旧继承值先解析并保留原来实际生效的策略，不是全部改为observe。双链路强制recover期间，仍保留下面原来的明确策略，关掉双链路会恢复它。批量中只要有账号双链路仍开着，就锁住守护选择；需要统一改守护，先明确勾选关闭双链路。
...
| 票据单次请求预算 | 账号→票据工作台→目标与尝试，可填5–300秒，默认25 | 双链路两阶段共用；不是每个阶段各120秒。导入默认值和勾选式批量也支持 |
| 动态API取号预算 | 当前代码8秒 | 取一条代理要在这个窗口内完成，包含在采集流程预算内 |
| 自动轮次预算 | 派发窗口30秒；全局默认最多60秒、单号默认30秒，随超时延长 | 不用手动设置；停止派发后给已派发请求留完整预算，未派发留到下轮 |

另外，手动参考IP查询最多有11秒预算；所以“票据请求结束”到“这一轮IP日志写完”可能还要多等一会。测试代理按钮本身整体最多20秒，与模型采集不是同一个超时。

其它保护不会全部跟着调大：取号8秒、数据库/缓存短预算、传输层建连/TLS/等待响应头仍各自受限。日志会区分“单次请求超时”“取号接口超时”“连接/握手/响应头超时”。采集锁自动变成单次超时+20秒；若你另行把共享业务槽TTL设得很短，系统会提示增大`gateway.concurrency_slot_ttl_minutes`或降低采集超时，避免锁先过期而并发失控。默认共享TTL足够，不需要为此额外配置。

填写方式：单号打开账号编辑，在票据工作台开启后找到“单次采集超时”，填例如120，使用账号原“更新”保存。批量先勾选工作台主开关修改并开启，再勾选“单次采集超时”，填值后用原批量保存。未勾选不修改，共同值回填、不同值留空。导入默认值只控制以后新增账号；已有号用批量改。修改账号票据规则沿原逻辑可能使旧票失效并重采。
...
两个设置不是同一个。题目超时只管题目；bh.058票据工作台可单独填5–300秒，默认25，双链路共享。采票可填120，不能填600；改Nginx也不能覆盖后端这项时限。

### 日志同时有“取消”和“资格暂停”，哪个是原因？

新版分开显示：历史结果说明那次为何结束，**当前暂停**说明此刻账号为什么不能采集（例如上游限流，并显示最早恢复时间）。两者可能不同时发生，不能从当前限流推断上一次一定因限流失败。后台收到关窗、网络断开和点击停止都体现为连接结束，因此合并标作“采集连接已断开或已停止”，不假装能够识别具体用户动作。

点“采集失败”统计卡片→对应账号→逐次尝试，看失败阶段、HTTP、请求/响应模型、耗时与单次上限。`API`是取号代理名称，不是错误码。`复用IP`指此前成功采集代理的复用，不代表这个IP导致失败；该参考IP不是业务复验的出口。若写着“上次已执行请求”，说明本次停止前没有再次请求上游，下面保留的是前一轮的真实诊断。

旧记录只保存“取消”或“资格不可用”的，升级不能还原缺失细节，不将它们自动改成超时。普通超时、断连不等于降智，不直接开关调度；明确长度信号按原规则处理。
...
| 事情 | 当前行为 |
| --- | --- |
| 新号第一次显示的长度 | 统一按导入模板；未保存过模板时用升级前有效默认值初始化模板，无历史值才用内置292 |
...
| 旧调用方不提交票据字段，且没有保存过模板 | 也复制归一后的导入模板并与新账号原子创建，不再产生长期读取旧全局规则的新号 |
| 非独立OpenAI OAuth | 不套票据模板；影子、Agent Identity和API Key不是这套采票账号 |

“模板设置开启”也越不过网关总闸。要真正自动维护，还须账号资格、代理、Redis、模型等都满足前面的规则。

bh.057对已有号不套用今天的新号模板：独立配置保留原值，缺项用升级前冻结快照补齐；旧全局0次原本代表默认次数1，不能改成账号0无限。归一不改账号/全局版本号、票据键或调度，也不自动发起采集。网关旧API里的模型/长度/次数/守护参数改为写同一新号模板，不再暗改旧号；旧代理参数仍映射为默认采集代理。仅改模板不废弃旧票，显式改网关总闸/代理仍按原规则换代。

导入默认值页面只重排现有字段：桌面模型白名单与映射并排，5h/7d额度成两组；手机自动上下排列。没有增加新的默认字段，映射和白名单的保存内容不变。
