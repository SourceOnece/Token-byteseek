...
- [Codex号池逻辑流程图与操作手册](codex_pool_operator_guide.md)：说明调度、检测、票据、代理、双链路、模板、批量及双上游发布；包含bh.056成功IP复用/两阶段颜色、工作台统一保存及实际长度筛选，区分验证与未测范围。读取时机：配置号池、理解功能联动、核对新增账号/筛选/批量行为、升级或交接管理员时。
- [bh.026 到 bh.030 完整差异说明](bh026_to_bh030_comparison.md)：用通俗语言解释正式版 bh.026 到当前 bh.030 的新增能力、原因、上游差异和未同步项。读取时机：准备从 bh.026 升级、核对当前镜像与两个上游差异时。
