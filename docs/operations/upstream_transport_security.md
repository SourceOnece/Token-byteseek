...
Codex票据代理只用于合成采集，支持账号优先的固定、失败轮换、动态认证模板或每次调用HTTPS接口取号；未配置继承网关默认代理，不修改业务账号代理或TLS路由。取号端点及返回代理检查公网地址并固定已解析IP，不跟随重定向，不把OAuth凭据发给取号服务；取号URL加密、审计脱敏。显式会话占位符只在代理认证信息中展开，同次手动参考IP复用该次代理但仍是独立请求。显式代理测试不保存草稿或调用模型。原生WS守护收据属于物理握手，不强制更换传输；详见[票据代理与存储](../interfaces/codex_ticket.md#ticket_contract)。
