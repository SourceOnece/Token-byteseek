...
Codex票据代理只用于合成采集，支持账号优先的固定、失败轮换、动态认证模板或HTTPS接口取号；未配置继承网关默认代理，不修改业务账号代理或TLS路由。bh.056轮换/动态可选成功IP复用，默认关闭；成功代理记忆加密、按账号模型/凭据/配置隔离、24小时无成功清理，管理代理每次仍检查状态和地址，不继承陈旧密码。取号端点及返回代理检查公网地址并固定已解析IP，不跟随重定向，不把OAuth凭据发给取号服务；取号URL加密、审计脱敏。显式会话占位符只在代理认证信息中展开，复用成功记录时不重新生成SID；同次手动参考IP仍是独立请求，不保证真实出口相同。显式代理测试不读写成功记忆、不保存草稿或调用模型。原生WS守护收据属于物理握手；详见[票据代理与存储](../interfaces/codex_ticket.md#ticket_contract)。
...
CLI 必须直连采集 HTTPS 入口，或经过不终止 TLS 的 SSH/TCP 转发；经 Nginx/CDN 终止 TLS 会采到中间层握手。使用生成的临时 CA 时，客户端只为本次进程增加 CA 信任，不永久禁用证书验证。必须先有管理员创建的有效采集会话：只连接端口、证书校验失败或 CLI 登录成功均不能证明已有采集记录。采集端返回的是诊断 JSON，不是模型 SSE，CLI 随后报协议解析错误并不等于采集失败，应以后台记录为准。
