...
Codex票据账号级动态代理只用于合成采集，未配置时继承网关采集代理，不修改业务账号代理或TLS路由。显式会话占位符只在代理认证信息中展开，同次手动参考IP仍是独立请求。原生WS守护收据属于物理握手，不强制更换传输；详见[票据账号覆盖与守护](../interfaces/codex_ticket.md#account_overrides_watchdog)。
