...
bh.046新增独立的票据长度调度来源：手动/自动采集合格并存票成功、或业务守护明确合格长度可开启；明确配置降智长度关闭，其它维持。它不写下述题目结果，所以“上次题目降智”与“后来获票开调度”可以同时存在。旧题目测试的版本保护仍生效，不用迟到结果覆盖更新的账号状态。规则与保护见[长度调度](../interfaces/codex_ticket.md#length_scheduling)。

管理员可选中独立 OpenAI OAuth/API Key 账号执行批量题目测试。使用指定模型、reasoning.effort、题目和单一大小写敏感的包含关键词，仅完整可见回答参与判定：命中为“满血”，未命中为“降智”，网络/认证/空回答/不完整响应为“测试失败”。这些标签是管理员自定义关键词结果，不是模型能力的客观认证。管理员必须明确确认调度影响；满血设置 schedulable=true，只有降智设置 false；测试失败保留原 schedulable（不自动关闭也不自动开启），不清除 inactive/error、到期或额度门禁。未选、跳过、取消和配置已变化的账号不改调度，已完成的结果不会因取消余下任务自动回滚。

结果独立保存到 codex_quality_tests，每账号只保留最近一条，不把长题目/回答加入账号 extra、调度缓存或用户接口。数据库租约阻止多实例同时测试同账号，有效结果原子保存；仅满血/降智更新 schedulable/outbox 并刷新调度快照，失败保留账号行及 updated_at 不变，仍写最近记录和定时历史。仓储返回“结果已接受”与 scheduling_applied 分开，失败不得因没有调度写入被误记 stale；输入快照发生变化则不应用旧结果。新批次每实例单批、并发 1–5（默认 3）、最多 500 个账号、每账号 timeout_seconds 可设 10–3600 秒，省略/0 时默认 120 秒；账号租约同步设置为超时加 30 秒。测试消耗上游额度。前端经管理员 SSE 获取结果并支持停止未完成测试，满血率分母是满血+降智+失败，跳过/取消/过时结果不计入。原单账号连接测试及定时恢复逻辑保持不变，本功能不添加永久禁止管理员手工改调度的门禁。

专用上下文只在此入口存在：抑制原探针额外写用量快照、401 永久错误和 429 状态恢复，调度修改统一交给结果存储。普通连接测试、定时测试和网关请求仍走原分支。完整回答支持 Responses delta 或 completed 中的最终 assistant output_text；最多读取 8 MiB SSE、单事件 1 MiB、可见回答 128 KiB，不完整/空回答/超限均记为失败，但不修改调度。题目不超过 16000 字符、关键词 200 字符，普通审计只记动作不复制题目。管理员列表默认读取摘要，单账号 detail=true 才读取完整输入输出。

账号 updated_at 会被正常用量更新刷新，因此先比较凭据、代理、账号状态/开关、TLS/客户端规则等实际配置，只有这些保持相同才采用最新 updated_at 做最终 SQL CAS；CAS 冲突保留 stale 结果，不改调度。运行期间外部策略或管理员仍可能改变资格，满血只证明该账号本次模型/题目/档位的关键词结果，不保证所有模型或 transport 均可用。关闭浏览器/弹窗会取消手动在途测试而不回滚已完成结果；应用异常退出后账号租约按超时加 30 秒到期，重启不自动继续手动批次。

bh.046按用户新要求统一full绿、degraded红、failed黄（替代此前满血蓝），包括账号列、检测/定时、详情、规则及仪表盘满血率/数量；统计点击才显示分类结果，回答默认折叠。进度条按实际结果数/固定账号数计算，取消不伪装100%。账号管理quality_status在数据库Count/分页前筛选并传至全选/批量/导出；仪表盘读取检测资格账号最近结果，不读取回答或发起检测，与单轮满血率分开。

手动/定时表单共用三角、方形、圆形及简短规则：满血开调度、降智关调度、失败保持；bh.046满血三角同步绿色。关键词规则、浏览器明文配置记忆和统计口径按需展开；额度、调度确认与其它资格限制保持可见，原payload、确认、资格、判定和调度不因配色变化而变更。

bh.041 将这三种规则合为连续面板，放大状态/动作并统一几何、说明、统计和卡片内距；手动/定时的模型与思考成组、协议/并发/超时成组，输入实例与请求不变。手机纵排，详情元信息用网格，表格密度保持。

手动批量弹窗把模型、协议、思考等级、题目、关键词、并发和超时按管理员 ID 存入当前站点的浏览器 localStorage，重新打开或刷新可恢复。保存字段有版本化键及类型/范围检查，损坏字段回退原默认值，存储不可用不阻止测试；自定义模型在上游目录加载失败时仍可显示。账号选择、执行结果和调度确认不持久化，每次打开都重新确认；切换管理员后加载各自配置并取消旧请求，不把前一管理员题目带入新草稿。题目/关键词为本机明文，不跨设备同步；清浏览器数据后恢复默认。

bh.032 的失败不改调度规则对手动批量、周期计划、计划“立即检测”共同生效；不回写旧失败结果、不自动打开旧版本已关闭的账号。失败仍参与原满血率分母，普通网关或连接测试的认证错误、限流、额度门禁不受此规则影响。

<a id="codex_quality_schedules"></a>
## Codex 定时检测计划

Codex 题目定时计划独立于旧 scheduled-test-plans。管理员指定固定账号 ID 集合、题目/关键词/模型/思考等级/超时、并发、间隔 1–43200 分钟和历史保留 1–100 轮（默认 30），必须确认调度影响。全选为当前搜索范围内符合条件的固定集合，最多 500 个独立 OpenAI OAuth/API Key 账号，不包含今后新导入账号；不可选择影子、Agent Identity 或其他平台。首次在保存/重新启用的间隔之后执行，下一轮从前轮完成后计时，不追补错过的多轮。

现有 ScheduledTestRunnerService 启停一个独立 15 秒轮询循环，使用 AccountTestService 的同实例批次锁，原单账号 cron 行为保持不变。数据库 FOR UPDATE SKIP LOCKED 领取到期计划、写运行快照和 60 秒计划租约；每 15 秒续租，派发每个账号前再校验计划。进程关闭取消在途任务并有界等待；崩溃后新实例可在租约到期领取，旧轮次标记 interrupted，账号租约仍避免重复调用。整轮超时按账号数/并发×（单号超时+30 秒）再加 60 秒计算。

暂停或编辑先锁计划并撤销 active_run_id，旧 worker 的账号结果提交会在共享计划锁下核验 enabled、运行 ID、轮次状态与租约；撤销后不得再更新调度。已经提交的结果不回滚。在途请求最多到下一次续租被取消，尚未派发的任务不会继续执行。原最近结果、schedulable、scheduler outbox 与定时历史结果使用同一 SQL 事务语义提交；跳过/取消/存储异常由 runner 补记，ON CONFLICT 不覆盖已提交事实。

codex_quality_schedules 保存计划，codex_quality_runs 保存每轮冻结配置，codex_quality_run_results 保存逐账号回答。每轮完成后按该计划 keep_runs 清理旧轮次及关联回答；降小保留数在下一轮完成时生效。UI 每 15 秒刷新计划/历史摘要，具体回答按分类每页 20 条读取。停止浏览器不停止已启用的定时计划；暂停需显式管理操作。历史只对管理员开放，题目不写通用审计正文。停用不删除历史，正常周期可能仍受上游额度、模型档位支持和网络故障影响。

计划可单次手动执行，与 enabled 周期开关独立：manual_requested_at 登记一次待执行请求，runner 领取后清空，并在轮次保存 trigger_source=manual；重复点击在领取前合并，完成后按原间隔计时，不会自动启用已暂停计划。API Key 的 api_protocol 单选 Responses/Chat Completions，未指定时兼容账号配置，OAuth 始终 Responses，不轮测多协议。

管理员可点击计划卡片删除按钮，再在显示计划名称的弹窗点击确认删除。DELETE 必须携带 confirm_delete=true；未确认或无效 ID 返回 400，不存在返回 404，数据库失败返回 500。只删除该计划及外键级联的每轮历史和回答，不删除账号或最近检测记录，不回滚已生效的调度开关；界面无撤销，恢复需已有备份。

删除与在途结果以计划行锁串行化。计划有效时结果提交持有共享锁，先完成的结果保留其调度影响；先删除/暂停/编辑时，旧结果只释放所属账号测试租约，不改最近结果或调度、不重建历史。补记跳过结果也锁定计划，避免级联删除与外键插入竞争。runner 派发前检查和心跳识别计划消失，停止剩余检测；已发到上游的请求无法保证即时撤销，仍可能消耗额度。删除成功后前端使旧列表请求失效并清除对应详情，避免慢响应让已删卡片重新出现。
