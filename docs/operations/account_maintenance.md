...
检测模块文字色按 bh.017 用户要求为 full 蓝、degraded 红、failed 黄；统计点击才显示分类结果，回答默认折叠。进度条按实际所有结果数/本轮固定账号数计算，取消不伪装 100%。账号管理 quality_status 筛选在数据库 Count/分页前执行，并传递至全选、按筛选批量编辑和导出；仪表盘聚合未删除 OpenAI OAuth 账号的最近结果，不读取回答或发起检测，与单轮满血率分开。

<a id="codex_quality_schedules"></a>
## Codex 定时检测计划

Codex 题目定时计划独立于旧 scheduled-test-plans。管理员指定固定账号 ID 集合、题目/关键词/模型/思考等级/超时、并发、间隔 1–43200 分钟和历史保留 1–100 轮（默认 30），必须确认调度影响。全选为当前搜索范围内符合条件的固定集合，最多 500 个独立 OAuth 账号，不包含今后新导入账号；不可选择影子、Agent Identity 或其他平台。首次在保存/重新启用的间隔之后执行，下一轮从前轮完成后计时，不追补错过的多轮。
