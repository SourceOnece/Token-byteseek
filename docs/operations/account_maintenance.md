...
检测模块文字色按 bh.017 用户要求为 full 蓝、degraded 红、failed 黄；统计点击才显示分类结果，回答默认折叠。进度条按实际所有结果数/本轮固定账号数计算，取消不伪装 100%。账号管理 quality_status 筛选在数据库 Count/分页前执行，并传递至全选、按筛选批量编辑和导出；仪表盘聚合未删除且符合检测资格的 OpenAI OAuth/API Key 账号的最近结果，不读取回答或发起检测，与单轮满血率分开。

bh.040 手动/定时表单共用蓝三角、红方形、黄圆形的简短规则：满血开调度、降智关调度、失败保持。关键词规则、浏览器明文配置记忆和统计口径按需展开；额度、调度确认与其他资格限制保持可见。结果卡片/统计入口用短标签和图形提示，回答仍折叠；只改文案与呈现，原 payload、确认、后台资格、判定与调度不变。
