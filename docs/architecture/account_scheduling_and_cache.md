...
bh.058新增账号单次采集超时（默认25秒、5–300秒），只影响后台/手动采集，不改变业务请求超时或调度三态。自动轮次、采集槽租约与显示时间同步延长，预算不足不发起注定被截断的下一次；共享业务槽TTL过短明确停止，不通过放宽并发补偿。只读升级补默认不换票据键。当前暂停原因与历史失败分开展示，详见[采集与冷却](../interfaces/codex_ticket.md#collection_policy)。
