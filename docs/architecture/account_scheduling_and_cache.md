...
bh.046把采集规则集中到账号，自动与手动采集均只在本地资格副本中忽略`schedulable=false`，不修改真实调度开关；账号状态、到期、限流等保护仍保留。可选连续采集异常冷却只门控该账号的对应模型，不写scheduler outbox或质量结果；冷却结束允许再采集，成功也不解除管理员/题目检测的停调。采集并发槽、0不限尝试的公平轮转、静态票据筛选与默认值见[采集与冷却](../interfaces/codex_ticket.md#collection_policy)。此例外不能扩散到普通业务调度。
...
bh.043修复初筛摘要缺OAuth令牌时误判缺票。bh.044补上述非令牌资格字段后，目标OAuth票据检查仍按同ID读取完整账号，仅在局部使用真实凭据索引原加密票据；旧摘要补全确认同ID为Agent Identity时恢复原豁免，其它不支持类型不借票放行。常规选后补全/资格复查保留。基础（含负载批量）/高级调度均须覆盖“摘要无Token但当前票有效”，并验证生产Redis投影资格一致性。关闭票据开关及非目标路径不因票据功能额外读取；细节与边界由票据契约拥有。
