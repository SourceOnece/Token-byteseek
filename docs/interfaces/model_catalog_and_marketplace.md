...
bh.061 补 GPT-6 Sol/Luna、Claude Opus 5.5 与 Grok 4.7 目录及独立价卡；Sol/Luna 非 none 档请求按映射后的实际型号去掉不支持的采样/logprobs 字段，Astra 保持原规则。新型号的动态目录缺失时使用同产品回退，不能借用旧 Opus 5/GPT 型号价格；渠道显式价格、免费缓存和 Fast 倍率仍优先。新目录不会自动修改已有账号/分组白名单，Grok 默认别名仍指向原配置或 4.6。独立上游 Codex manifest 不是本批覆盖范围。

bh.026 新增 Image 2.5 Flare/Sunburst 及 2026-09-08 快照的精确目录与兜底价；目录滞后时不得回退旧图片模型。原生 Images 明确返回图片缓存 token 时，从非缓存图片输入扣除并单独按图片缓存价计算，缺少拆分时不猜测。既有模型价卡和渠道/分组优先级不变，创作台仍按原尺寸单价计费；市场价格组件继续沿用 Standard 绿色、Fast 黄色设计。

价格文件定时同步同时按内容哈希检查本地 fallback/override；远端同步失败不阻止本地层检查，未配置远端但配置本地层时仍启动检查。文件变动重建内存目录，保留分组/渠道的原有优先级；无效 JSON 保留旧目录并等待下轮，不把文件半写状态当新价格。文件删除视为移除该层，读取期间内容变化则有界重试。该能力只改变配置生效方式，不增加收费倍率。
...
创建普通 API Key 时，分组选择增加 Anthropic、OpenAI、国产模型、其他四类筛选，分类依据真实平台而非名称/展示品牌；Qoder 与 OpenCode 归入其他。分类先受当前个人/团队主体及指定套餐允许分组的交集限制，不扩大权限、不自动选择具体分组，也不改创建 API。编辑既有 Key 和复合 Key 的跨平台映射不使用此筛选。账号编辑则补回详情中仍绑定但已停用的分组，让管理员能明确移除；不把未绑定的停用组加入可选清单。
