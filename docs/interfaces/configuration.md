...
票据采集的 models 与 degraded_signal_length 由管理员“网关服务 → OpenAI → 票据采集”独立保存，models 每行一个模型ID，默认Astra/Sol，支持1–100个；降智信号0关闭或指定状态头字节数，只作提示。它们不属于公开配置，具体兼容与模型门控范围见[票据契约](codex_ticket.md#ticket_contract)。
...
## 领域配置
<a id="team_invitation_limits"></a>
### 团队邀请频率

管理端“功能开关 → 团队功能”保存 team_invitation_cooldown_seconds（1–86400 秒，默认 60）与 team_invitation_hourly_limit（1–10000 次，默认 20），无迁移、无重启要求。管理设置 GET/PUT 提供两项，旧更新请求省略字段保持当前值；公开设置不新增字段。

TeamService 在邀请和重发前一次批量读取两键，无进程缓存；读库失败沿用邀请服务 fail-close，缺键或无效历史值回退默认。Redis 沿用同团队+邮箱冷却与每团队首请求起算的一小时窗口：变更小时上限立即作用下次判断，不清零已有次数；已写入的冷却按原 TTL 结束，下一次允许发送后使用新间隔。0 不表示禁用限制。收件邮箱仍标准化后以摘要作为键，不在 Redis key 放明文邮箱。

限流仍先于实际发信记录；后续 SMTP/创建邀请失败可能占次数。只影响本平台团队邀请，不改变上游 Codex 邀请或其他通知发信限制。
