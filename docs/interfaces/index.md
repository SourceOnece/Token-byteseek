...
- [Codex 可选票据](codex_ticket.md)：默认关闭的票据采集、私有存储、注入与号池隔离边界。读取时机：维护 Codex 票据开关、后台探测、缓存、生命周期及转发时读取。
...
- [上游账号能力矩阵](upstream_account_matrix.md)：十一平台、七类账号和全部公开网关协议的正式支持、兼容保留与不支持边界。读取时机：新增平台/账号类型、修改创建导入校验、路由分派或能力承诺时读取。
...
- [OpenCode 上游](opencode_upstream.md)：Zen/Go 模式、模型协议规则、租户隔离会话和统一用量查询。读取时机：修改 OpenCode 账号创建/编辑、三协议转换、模型目录、用量和 429 时读取。
