...
管理员用户列表的批量删除仅组合现有单用户删除端点，弹窗确认后固定 ID 快照并串行执行；成功项取消选择，失败项保留，新选中的用户不会被在途批次删除。后端继续拒绝删除管理员，并保持用户/API Key 的原事务和失效规则。此功能不增加可绕过管理员鉴权的批量后端路由。
...
团队邀请及重发共用跨实例 Redis 限流。管理员可在团队功能设置中自定义同团队同邮箱的间隔与每团队每小时总次数，默认 60 秒/20 次；设置及冷却生效边界见[团队邀请频率](../interfaces/configuration.md#team_invitation_limits)。读取限流配置或 Redis 失败时拒绝发送，不能因依赖故障放行。
