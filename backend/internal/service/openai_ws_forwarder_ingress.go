...
func (s *OpenAIGatewayService) ProxyResponsesWebSocketFromClient(
	...
	refreshIngressRouteState := func(payload openAIWSClientPayload) {
		...
		if useHTTPBridge {
			// 账号粘性可以共用，但 HTTP 桥不能继承其他连接的原生 WS 回合状态或连接绑定。
			return
		}
	...
	if useHTTPBridge {
		...
		for turn := 1; ; turn++ {
			...
			if bridgeTurnState := strings.TrimSpace(result.ResponseHeaders.Get(openAIWSTurnStateHeader)); bridgeTurnState != "" {
				// 后续轮次只保留本桥的上游状态，不能通过会话哈希发布给其他独立桥。
	...
	wsHeaders, ticketSession, buildHdrErr := s.buildOpenAIWSHeaders(
		ctx,
		c,
		account,
		token,
	...
	baseAcquireReq := openAIWSAcquireRequest{
		ticketReceipt: ticketSession.ticketReceipt,
		Account:       account,
		WSURL:         wsURL,
		Headers:       wsHeaders,
	...
	sendAndRelay := func(turn int, lease *openAIWSConnLease, payload []byte, payloadBytes int, originalModel string, routingModel string, imageBillingModel string, imageSizeTier string, imageInputSize string, requestedReasoningEffort *string) (*OpenAIForwardResult, error) {
		...
		for {
			...
			if readErr == nil && lease.conn != nil {
				lease.conn.ticketReceipt.observeJSON(upstreamMessage, "", openAIWSPayloadStringFromRaw(payload, "model"))
			}
			if readErr != nil {
				lease.MarkBroken()
				return nil, wrapOpenAIWSIngressTurnError(
	...
	for {
		...
		if nextPayload.promptCacheKey != "" {
			...
			updatedHeaders, ticketSession, updHdrErr := s.buildOpenAIWSHeaders(
				ctx,
				c,
				account,
				token,
			...
			if updHdrErr != nil {
				...
				baseAcquireReq.Headers = updatedHeaders
				baseAcquireReq.ticketReceipt = ticketSession.ticketReceipt
			}
		}
