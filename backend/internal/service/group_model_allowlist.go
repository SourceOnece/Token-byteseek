...
// CloneGroupModelAllowlist 防止请求投影、复制分组与共享鉴权缓存共用可变切片。
func CloneGroupModelAllowlist(cfg GroupModelAllowlist) GroupModelAllowlist {
	return GroupModelAllowlist{Enabled: cfg.Enabled, Models: append([]string(nil), cfg.Models...)}
}
...
// 回退分组也必须校验客户端原名，不能因 Key/渠道内部映射而误拒绝公开别名。
func validateGroupModelAllowlistForSelection(ctx context.Context, group *Group, model string) error {
	if group == nil || !group.ModelAllowlistEnabled() {
		return nil
	}
	if trace, ok := APIKeyModelRedirectTraceFromContext(ctx); ok {
		model = trace.ClientModel
	}
	if group.ModelAllowlist.Allows(model) {
		return nil
	}
	return &GroupModelUnsupportedError{Platform: group.Platform, RequestedModel: model, AvailableModels: append([]string(nil), group.ModelAllowlist.Models...)}
}
