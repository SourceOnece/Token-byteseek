...
func TestNormalizeDeepSeekResponsesRequestBodyAliasesInputImageURL(t *testing.T) {
	...
	// 用户工具参数和非媒体数据是应用负载，不能被兼容清洗器递归修改。
	t.Run("opaque_tool_arguments_preserved", func(t *testing.T) {
		body := []byte(`{"model":"deepseek-flash","input":[{"type":"function_call","arguments":{"type":"image_url","image_url":"private-data"}}]}`)
		got := normalizeDeepSeekResponsesRequestBody(native, body)
		require.Equal(t, "image_url", gjson.GetBytes(got, "input.0.arguments.type").String())
		require.False(t, gjson.GetBytes(got, "input.0.arguments.url").Exists())
	})
