...
func ollamaResetTimePtr(t time.Time) *time.Time { return &t }
...
func TestOllamaCloudUsageExhaustionResetAt(t *testing.T) {
	...
	at := func(h, m int) *time.Time {
		return ollamaResetTimePtr(now.Add(time.Duration(h)*time.Hour + time.Duration(m)*time.Minute))
	}
	...
	okSnapshot := func(data *OllamaCloudUsageData) *OllamaCloudUsageSnapshot {
		return &OllamaCloudUsageSnapshot{
			...
			Data:      data,
			FetchedAt: ollamaResetTimePtr(now.Add(-time.Minute)),
		}
	}
	...
	tests := []struct {
		...
		},
		{
			...
			snapshot: func() *OllamaCloudUsageSnapshot {
				...
				s.FetchedAt = ollamaResetTimePtr(now.Add(-3 * time.Hour))
				return s
			}(),
...
func TestOllamaCloudUsageExhaustionResetAtMissingFetchedAt(t *testing.T) {
	...
	snapshot := &OllamaCloudUsageSnapshot{
		...
		Data: &OllamaCloudUsageData{
			FiveHour: &OllamaCloudUsageWindow{UsedPercent: 100, ResetAt: ollamaResetTimePtr(now.Add(2 * time.Hour))},
		},
	}
