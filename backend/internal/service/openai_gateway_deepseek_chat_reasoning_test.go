...
func TestForwardResponses_DeepSeekChatFallbackKeepsCachedReasoningContent(t *testing.T) {
	...
	key := scopedResponsesReasoningKey(responsesReasoningScope(c, account), "item_enc1")
	svc := &OpenAIGatewayService{
		cfg:          deepSeekChatFallbackTestConfig(),
		httpUpstream: upstream,
		cache:        &reasoningHitCache{values: map[string]string{key: "cached thinking"}},
	}
