...
func TestOpenAIGatewayService_Forward_WSv2_ExecutionScopeUsesOriginalIdentity(t *testing.T) {
	...
	// 身份变化会让连接退出复用池；每次拨号须返回新连接，不能重用已关闭的桩对象。
	pool.setClientDialerForTest(&openAIWSScopeFreshDialer{})
	defer pool.Close()
...
type openAIWSScopeFreshDialer struct{}

func (*openAIWSScopeFreshDialer) Dial(context.Context, string, http.Header, string, *tlsfingerprint.Profile) (openAIWSClientConn, int, http.Header, error) {
	handshake := http.Header{}
	handshake.Set(openAIWSTurnStateHeader, "turn-state-from-upstream")
	event := []byte(`{"type":"response.completed","response":{"id":"resp_scope","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`)
	return &openAIWSCaptureConn{events: [][]byte{event, event, event}}, 0, handshake, nil
}
