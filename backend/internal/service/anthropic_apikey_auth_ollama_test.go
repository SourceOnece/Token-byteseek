...
func TestSetAnthropicAPIKeyAuthHeader_OllamaCloudForcesBearer(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Equal(t, tt.wantAuth, getHeaderRaw(header, "authorization"), "Authorization 头不符")
...
func TestSetAnthropicAPIKeyAuthHeader_CNAdaptiveBaseURL(t *testing.T) {
	...
	require.Equal(t, "Bearer kimi-key", getHeaderRaw(header, "authorization"))
	...
	require.Empty(t, getHeaderRaw(header, "authorization"))
...
func TestGatewayService_AnthropicPassthrough_OllamaCloudBearer(t *testing.T) {
	...
	require.Equal(t, "Bearer ollama-cloud-key", getHeaderRaw(ollamaReq.Header, "authorization"))
	require.Empty(t, getHeaderRaw(ollamaReq.Header, "x-api-key"), "Ollama Cloud 上游不得再发 x-api-key")
	require.NotContains(t, getHeaderRaw(ollamaReq.Header, "authorization"), "inbound-token")
...
func TestOpenAIGatewayService_NativeAnthropicBridge_OllamaCloudBearer(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, "Bearer ollama-cloud-key", getHeaderRaw(req.Header, "authorization"))
