...
// DefaultOpenCodeGoModelIDs 保存本批固定上游版本的模型预设，
...
// OpenCodeGoProtocolRule 以精确名称或尾部通配符匹配模型，第一条命中优先。
type OpenCodeGoProtocolRule struct {
	Pattern  string `json:"pattern"`
	Protocol string `json:"protocol"`
}

// DefaultOpenCodeGoProtocolRules 为 Go 账号的默认原生协议规则。
...
// 新平台只接收 API Key；只在创建时补默认值，编辑旧账号不隐式改写模式。
func normalizeOpenCodeAccountCredentials(account *Account, isCreate bool) error {
	if account.Type != AccountTypeAPIKey {
		return infraerrors.BadRequest("OPENCODE_ACCOUNT_TYPE_INVALID", "OpenCode accounts require API Key credentials")
	}
	if account.Credentials == nil {
		account.Credentials = map[string]any{}
	}
	mode := strings.TrimSpace(account.GetCredential("account_mode"))
	if mode == "" {
		if isCreate {
			account.Credentials["account_mode"] = AccountModeGo
		}
	} else if mode != AccountModeGo && mode != AccountModeZen {
	...
	} else if protocol != APIProtocolAdaptive && !isNativeOpenCodeGoProtocol(protocol) {
...
// @project-doc docs/interfaces/opencode_upstream.md#opencode_protocol_routing
...
func openCodeGoQuotaURL(baseURL string) string {
	...
	if !openAIBaseURLHasVersionSuffix(parsed.Path) {
