...
// 通用评分与诊断使用同一个窗口解析器；其他平台和 API Key 不读取 Codex 窗口。
func TestAdvancedSchedulerCanonicalResetIsolation(t *testing.T) {
	now := time.Now()
	sessionEnd := now.Add(3 * time.Hour)
	quotaEnd := now.Add(10 * time.Minute).Truncate(time.Second)
	for _, platform := range []string{PlatformOpenAI, PlatformAnthropic, PlatformGemini, PlatformGrok} {
		for _, kind := range []string{AccountTypeOAuth, AccountTypeAPIKey} {
			a := &Account{ID: 1, Platform: platform, Type: kind, SessionWindowEnd: &sessionEnd, Extra: map[string]any{"codex_5h_reset_at": quotaEnd.Format(time.RFC3339)}}
			end, ok := advancedSchedulingResetWindowEnd(a, now)
			require.True(t, ok)
			want := sessionEnd
			if platform == PlatformOpenAI && kind == AccountTypeOAuth {
				want = quotaEnd
			}
			require.WithinDuration(t, want, end, time.Second)
		}
	}
	a := &Account{ID: 1, Platform: PlatformOpenAI, Type: AccountTypeOAuth, Extra: map[string]any{"codex_5h_reset_at": quotaEnd.Format(time.RFC3339)}}
	b := &Account{ID: 2, Platform: PlatformOpenAI, Type: AccountTypeOAuth, SessionWindowEnd: &sessionEnd}
	weights := GatewayAdvancedSchedulerScoreWeightsView{Reset: 1}
	candidates, _ := scoreAdvancedSchedulerCandidates([]*Account{a, b}, nil, nil, weights, advancedSchedulerSelectionInput{}, now)
	require.InDelta(t, 1, candidates[0].score, 1e-9)
	require.InDelta(t, 0, candidates[1].score, 1e-9)
}
