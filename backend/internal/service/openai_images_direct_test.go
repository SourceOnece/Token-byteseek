...
func TestCodexDirectImagesStreamRejectsPlainJSON(t *testing.T) {
	body := []byte(`{"model":"gpt-image-2.5-flare","prompt":"draw","stream":true}`)
	c, _ := newOpenAIImagesTestContext(t, body)
...
func TestCodexDirectImagesStreaming(t *testing.T) {
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			body := []byte(`{"model":"gpt-image-2.5-flare","prompt":"edit","images":[{"image_url":"data:image/png;base64,AA=="}],"stream":true,"response_format":"url"}`)
			if test.name == "partial_requested_multiple" {
				body = []byte(`{"model":"gpt-image-2.5-flare","prompt":"edit","n":2,"images":[{"image_url":"data:image/png;base64,AA=="}],"stream":true,"response_format":"url"}`)
			}
			c, rec := newOpenAIImagesTestContext(t, body)
			...
			if test.wantCount > 0 {
				...
				require.Equal(t, 20, result.Usage.ImageOutputTokens)
				if !test.wantError {
					require.Equal(t, "gpt-image-2-codex", upstreamResponseModelObserverFromContext(c).Model())
				}
			}
...
func TestCodexDirectImagesAccountTestAndWhitelist(t *testing.T) {
	...
	// 本 fork 模型选择由管理 handler 的配置模型列表负责，不引入另一套发现接口。
	account := directImagesTestAccount()
	account.Credentials["model_mapping"] = map[string]any{"gpt-image-2.5-flare": "gpt-image-2.5-flare"}
	require.Contains(t, account.GetConfiguredRequestModels(), "gpt-image-2.5-flare")
	require.NotContains(t, account.GetConfiguredRequestModels(), "gpt-image-2.5-sunburst")
}

// 管理端探测与正式转发在端点不可用时都回退一次，并保留 OAuth 身份及 TLS 配置。
func TestCodexDirectImagesAccountTestFallback(t *testing.T) {
	for _, status := range []int{404, 405, 401, 429, 503} {
		t.Run(fmt.Sprint(status), func(t *testing.T) {
			body := []byte(`{"model":"gpt-image-2.5-flare","prompt":"draw"}`)
			c, _ := newOpenAIImagesTestContext(t, body)
			upstream := &httpUpstreamRecorder{responses: []*http.Response{
				{StatusCode: status, Header: make(http.Header), Body: io.NopCloser(strings.NewReader(`{"error":{"message":"synthetic"}}`))},
				{StatusCode: 200, Header: make(http.Header), Body: io.NopCloser(strings.NewReader("data: {\"type\":\"response.completed\",\"response\":{\"output\":[{\"type\":\"image_generation_call\",\"result\":\"aGVsbG8=\"}]}}\n\n"))},
			}}
			...
			err := svc.testOpenAIImageOAuth(c, context.Background(), account, "gpt-image-2.5-flare", "draw")
