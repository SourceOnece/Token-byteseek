...
// 兼容不能把“同时满足”放宽成“任选一个”，也不能丢掉不同根联合组各自的必填字段。
func TestResponsesToAnthropic_AllOfAndRootConstraintsPreserved(t *testing.T) {
	raw := normalizeAnthropicInputSchema(json.RawMessage(`{
		"type":"object","properties":{"n":{"type":"integer"}},
		"allOf":[{"type":"object","properties":{"n":{"minimum":1}},"required":["n"]},{"type":"object","properties":{"n":{"maximum":5}}}],
		"anyOf":[{"type":"object","properties":{"a":{"type":"string"}},"required":["a"]}],
		"oneOf":[{"type":"object","properties":{"b":{"type":"string"}},"required":["b"]}]
	}`))
	root := requireObjectInputSchema(t, raw)
	var properties map[string]json.RawMessage
	require.NoError(t, json.Unmarshal(root["properties"], &properties))
	require.Contains(t, string(properties["n"]), `"allOf"`)
	require.NotContains(t, string(properties["n"]), `"anyOf"`)
	require.Contains(t, string(properties["n"]), `"minimum":1`)
	require.Contains(t, string(properties["n"]), `"maximum":5`)
	var required []string
	require.NoError(t, json.Unmarshal(root["required"], &required))
	require.ElementsMatch(t, []string{"a", "b", "n"}, required)
}
