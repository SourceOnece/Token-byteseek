...
func flattenAnthropicRootUnions(schema map[string]json.RawMessage) {
	...
	required := append([]string(nil), rootRequired...)
	for _, keyword := range present {
		...
		// 分支择一使用anyOf；根级properties与其它联合组同时成立，必须再用allOf合并。
		unionProperties := make(map[string]json.RawMessage)
		mergeAnthropicObjectBranchProperties(unionProperties, branches, "anyOf")
		if encoded, err := json.Marshal(unionProperties); err == nil {
			mergeAnthropicObjectBranchProperties(properties, []map[string]json.RawMessage{{"properties": encoded}}, "allOf")
		}
