...
//
// 调用位置：本地 API Key 认证中复合 Key 选组之后、Key 模型重定向之前——
...
func checkGroupModelAllowlist(c *gin.Context, apiKey *service.APIKey) {
...
func groupModelAllowlistErrorWriter(c *gin.Context) GatewayErrorWriter {
	...
	default:
		return func(c *gin.Context, status int, message string) {
			c.JSON(status, gin.H{"error": gin.H{"type": "invalid_request_error", "code": "model_not_allowed", "message": message}})
		}
	}
}
