...
func TestCodexTicketCacheRealRedis(t *testing.T) {
	...
	t.Cleanup(func() {
		_ = integrationRedis.Del(ctx, "private:codex-ticket:v1:"+key, "private:codex-ticket:v1:latest:"+key, "private:codex-ticket-lease:v1:"+key, "private:codex-ticket-lease:v1:round:"+key).Err()
	})
	a, b := NewCodexTicketCache(integrationRedis), NewCodexTicketCache(integrationRedis)
	require.NoError(t, a.SetLatest(ctx, key, `{"finished_us":200,"source":"manual"}`, 200, time.Minute))
	require.NoError(t, b.SetLatest(ctx, key, `{"finished_us":100,"source":"auto"}`, 100, time.Minute))
	newest, err := b.Get(ctx, "latest:"+key)
	require.NoError(t, err)
	require.Contains(t, newest, `"manual"`)
	...
	require.NoError(t, err)
	require.True(t, ok)
	value, err = a.LeaseValue(ctx, "round:"+key)
	require.NoError(t, err)
	require.Equal(t, "owner-a", value)
	ok, err = b.ReplaceLease(ctx, "round:"+key, "wrong-owner", "new-selection", time.Minute)
	require.NoError(t, err)
	require.False(t, ok)
	ok, err = a.ReplaceLease(ctx, "round:"+key, "owner-a", "owner-a", time.Minute)
	require.NoError(t, err)
	require.True(t, ok)
