...
func (r *accountRepository) accountListFilteredQuery(platform, accountType, status, search string, groupID int64, privacyMode string) *dbent.AccountQuery {
	...
	if search != "" {
		q = q.Where(dbaccount.Or(dbaccount.NameContainsFold(search), accountEmailContainsFold(search)))
	}
...
func (r *accountRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, platform, accountType, status, search string, groupID int64, privacyMode string) ([]service.Account, *pagination.PaginationResult, error) {
	...
	applyCodexTicketFilter(q, service.CodexTicketFilter(ctx), service.CodexTicketMatchedIDs(ctx))
...
func (r *accountRepository) ListAllWithFilters(ctx context.Context, platform, accountType, status, search string, groupID int64, privacyMode string) ([]service.Account, error) {
	...
	applyCodexTicketFilter(query, service.CodexTicketFilter(ctx), service.CodexTicketMatchedIDs(ctx))
...
// SetRateLimitedIfUnchanged 用同一条语句核对行版本和限流代次，防止异步探测覆盖换 Key、
// 管理员清除及新 429。仅更新成功后发送调度通知，调用者不能再作无条件限流写入。
