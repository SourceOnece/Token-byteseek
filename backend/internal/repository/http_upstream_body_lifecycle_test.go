...
// 两个公开入口均使用本地测试服务；TLS 采用测试证书，不依赖系统信任根。
...
func TestHTTPUpstreamConcurrentEarlyCloseDoesNotPoisonNextResponse(t *testing.T) {
	for _, withTLS := range []bool{false, true} {
		t.Run(fmt.Sprintf("tls=%v", withTLS), func(t *testing.T) {
			...
			// 后台进入 Read 后才执行 Close，此时服务端仍保持流打开。
			<-readStarted
			...
			// Close 返回后才结束旧响应，复现 Go 1.27 连接被旧读取阻塞的场景。
			close(finishFirst)
			<-firstFinished
			for i := 0; i < 8; i++ {
				resp, err := do(req)
...
// 保持基准可对照旧版：测完整 Do/read/Close 生命周期，避免网络耗时
// 掩盖每次尝试 context 与读取锁的成本。
