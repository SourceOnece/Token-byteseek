...
// Accounts Management
    accounts: {
      ticketCollect: {
        ...
        hint: '本次选中 {count} 个账号，按下方已保存模型列表分别采集。手动允许关闭调度的账号参与，不会打开其调度；其余资格与限流仍保留。沿用网关服务 OpenAI 配置，不先删除有效旧票。',
      ...
      quality: {
        ...
        warning: '本次选中 {count} 个账号。支持独立 OpenAI OAuth 与 API Key 上游，每号最多等待 {seconds} 秒；测试消耗上游额度，且会自动修改这些账号的调度开关。',
