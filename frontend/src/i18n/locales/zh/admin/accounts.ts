...
// Accounts Management
    accounts: {
      ...
      ticketDiagnostic: {
        "previousAttempt": "以下为上次已执行请求，本次未再请求上游",
        "budget": "单次上限 {seconds}s",
        "elapsed": "本次耗时 {seconds}s",
        "providerHTTP": "取号接口 HTTP {status}",
        "currentPause": "当前暂停：{reason}",
        "resumeAt": "最早恢复：{time}",
        "phases": {
          "eligibility": "资格检查",
          "queue": "等待并发",
        ...
        "network": {
          ...
          "dns": "DNS解析失败",
          "tls": "TLS证书校验失败",
          "connection": "网络/代理连接失败"
        },
      "reasons": {
        "proxy_timeout": "取号接口超时（独立上限8秒）",
        "transport_timeout": "连接、TLS握手或等待响应头超时",
        "concurrency_config": "共享业务并发槽TTL过短，请增大gateway.concurrency_slot_ttl_minutes或降低采集超时",
          "network": "网络或代理连接失败",
          "upstream": "上游返回错误",
          "invalid_ticket": "票据长度或格式不合格",
          "credential": "无法取得有效凭据",
          "storage": "数据库或票据缓存读写失败",
          "cancelled": "任务已取消；旧记录未区分具体原因",
          "proxy_config": "采集代理配置无效或解密失败",
          "proxy_provider": "取号接口失败或返回代理无效",
          "cooldown": "连续采集异常，等待冷却",
          "ineligible": "资格不可用；旧记录未细分原因",
          "account_changed": "账号凭据或票据配置已变化",
          "concurrency_busy": "等待并发槽，本次未请求上游",
          "backoff": "上游要求等待，稍后再采集",
          "business_proxy": "未绑定可用业务代理",
          "incomplete_response": "未收到完整成功响应或模型字段",
          "model_mismatch": "响应模型与请求模型不一致",
          "length_signal": "命中配置的降智长度",
          "timeout": "单次请求超时",
          "round_timeout": "自动轮次时间预算已到",
          "task_timeout": "任务上层请求超时",
          "client_disconnected": "采集连接已断开或已停止",
          "lease_lost": "采集任务锁已失效，停止剩余任务",
          "config_unavailable": "票据配置读取失联，已保护性暂停",
          "service_stopped": "服务停止或重启，采集中止",
          "config_disabled": "票据采集开关已关闭",
          "account_missing": "账号不存在",
          "unsupported_account": "账号类型不支持票据采集",
          "account_inactive": "账号已禁用或处于错误状态",
          "account_expired": "账号已到期且启用到期暂停",
          "account_overloaded": "账号处于上游过载保护期",
          "account_rate_limited": "账号处于上游限流期",
          "account_cooldown": "账号处于临时不可调度保护期",
          "model_unsupported": "账号未开放该模型",
          "attempt_limit": "已达到配置的尝试次数",
          "unknown": "未记录可识别的原因"
        }
      },
      ticketWorkbench: {
        ...
        fields: { attempt_timeout_seconds: '单次采集超时（秒）', target_length: '合格长度（字节）', degraded_signal_length: '异常长度信号（0关闭）', max_attempts: '最多尝试（0不限）', concurrency: '该账号采集并发（1–4）', cache_minutes: '缓存时间（分钟）', refresh_before_minutes: '提前续采（分钟）', retry_interval_seconds: '失败重试间隔（秒）', probe_interval_seconds: '自动检查间隔（秒）', failure_threshold: '连续采集异常（0关闭冷却）', cooldown_seconds: '冷却后重采（秒）' },
        timeoutHint: '5–300秒，默认25。取代理、采集与复验共用；调大将更久占用并发。',
      ...
      ticketPolicy: {
        ...
        saved: '票据配置已保存', guardRisk: '自动重采会废弃本次异常票据，恢复前该模型可能暂不可用；不重放回答。',
        source: { account: '账号代理', gateway: '网关代理' },
        modes: { on: '开启', off: '关闭' },
        proxyActions: { custom: '设置账号代理', inherit: '恢复网关代理' },
        guards: { off: '关闭业务守护', observe: '长度调度 · 不自动重采', recover_length: '长度信号 → 重采', recover_model: '模型不符 → 重采', recover: '任一异常 → 重采' },
      ...
      ticketCollect: {
        ...
        progress: '账号 × 模型', live: '最近 50 次尝试', runStatus: '批次状态',
        historyHint: '关窗/断连停止剩余采集，已获票和日志保留。完整日志见历史。',
      ...
      tickets: {
        lengthLabel: '实际 {actual} 字节，目标 {target} 字节',
        degradedSignal: '降智信号', openDetails: '查看 {model} 采集详情',
        source: { manual: '手动', auto: '自动' }, latestAt: '完成时间', currentTicket: '当前票据',
      ...
      quality: {
        ...
        rule: { full: '开调度', degraded: '关调度', failed: '保持调度' },
        protocol: 'API 上游协议', actualProtocol: '实际检测协议', protocolDefault: '沿用账号配置',
        poolRate: '号池满血率',
        timeout: '单号超时（秒）', dashboard: 'Codex 号池检测', dashboardHint: '每账号取最新结果；未检测不计入满血率。',
        detectedEmail: '账号邮箱', emailClaimHint: '令牌声明，仅展示；RT/授权码交换后获取，不代表授权有效。',
        email: '账号邮箱', emailUnavailable: '未获取邮箱', viewAnswer: '测试回答',
        clickCategory: '点击上方分类查看对应账号；点击满血率查看全部结果。', emptyCategory: '该分类暂无结果', allResults: '全部检测结果',
        schedule: {
          deleteTitle: '删除定时检测计划', deleteConfirm: '确认删除',
          deleteMessage: '永久删除计划与历史，取消待执行任务；运行任务发现删除后停止，已发请求仍可能扣额度。不可撤销。',
          deletePreserved: '保留账号、最近结果与已生效调度。',
          periodic: '周期检测', queued: '等待执行', stop: '停止本轮并暂停计划', manualRun: '手动发起', scheduledRun: '周期执行',
          title: 'Codex 定时检测', description: '服务端定时运行 · 关窗继续 · 逐轮留档',
          timingHint: '首次等待所设间隔；后续从上轮结束计时，不重叠。',
          create: '创建定时计划', name: '计划名称', interval: '检测间隔（分钟）', keep: '保留最近轮数', selected: '已选 {count} 个账号',
          selectAll: '全选可测账号', fixedSelection: '最多 500 个 · 固定名单，不含新增、影子与 Agent Identity',
          enabled: '已启用', paused: '已暂停', enable: '启用检测', pause: '暂停检测', runNow: '立即检测', confirm: '确认按上述规则调度并持续消耗额度；禁用、到期和额度限制仍有效。',
        ...
        warning: '已选 {count} 个 · 每号 {seconds}s · 消耗上游额度',
        disclaimer: 'OpenAI OAuth / API Key 可测；其他平台、影子与 Agent Identity 跳过。标签仅为关键词结果，不代表模型能力；未选账号不变。',
        model: '测试模型', effort: '思考等级', effortDefault: '上游默认（不传）', concurrency: '并发数量',
        prompt: '测试题目', keyword: '判定关键词', keywordHint: '只匹配完整可见回答，区分大小写；不是正则，不含题目或推理摘要。',
        rememberHint: '配置按管理员保存在本浏览器（题目/关键词为明文），清数据即清除；不记住账号与确认。',
        confirm: '确认按上述规则调度；禁用、到期和额度限制仍有效。',
        rate: '本批满血率', rateHint: '满血率 = 满血 ÷（满血 + 降智 + 失败）；排除跳过、取消、过时。',
        status: { full: '满血', degraded: '降智', failed: '测试失败', skipped: '已跳过', cancelled: '已取消', stale: '结果过时' },
        noAnswer: '暂无回答', schedulingOn: '调度已开 · 其他限制仍有效', schedulingOff: '调度已关', schedulingUnchanged: '调度未变',
        start: '测试并调度', stop: '停止测试', progress: '正在测试 {done} / {total}', completed: '测试结束 {done} / {total}', stopped: '已停止 {done} / {total} · 已生效调度保留',
      ...
      opencodeGo: {
        ...
        protocolRules: {
          invalid: '规则最多 64 条；模型名不能为空，只能使用末尾 * 通配。',
          title: '模型协议分流',
      ...
      cnProviders: {
        accountMode: {
          zen: 'Zen',
          zenDesc: 'OpenCode 按量付费网关。',
          go: 'GO',
          goDesc: 'OpenCode 订阅额度，包含 5 小时、周和月窗口。',
          title: '账号类型',
          payg: '按量付费',
          paygDesc: '消耗账户余额，按 Token 计费。余额不足自动冷却，充值后恢复。',
          minimaxPaygDesc: 'MiniMax 按量计费密钥，不提供余额查询。',
          coding: 'Coding Plan',
