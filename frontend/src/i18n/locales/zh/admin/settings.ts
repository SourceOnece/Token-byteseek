...
// Settings
    settings: {
      codexTicket: {
        models: '自定义采集模型', modelsHint: '每行一个完整上游模型 ID，支持多个模型，最多 100 个，重复项自动合并。保存后用于自动/手动采集及原有缺票检查；账号仍需支持该模型。',
        signalLength: '降智信号长度（字节）', signalHint: '0 关闭；例如 312。命中仅显示“降智信号”，不自动废票或改质量结果。',
        invalidModelsSignal: '请填写 1–100 个有效模型 ID（每行一个、不含空白）；降智信号为 0 或 6–8192 的整数。',
        mode: '失败重试策略', rotate: '未获票后切换代理', fixed: '固定代理重试',
        fixedProxy: '指定采集代理', addProxy: '添加采集代理', proxyName: '代理名称', proxyLabel: '代理 {index}',
        keepAddress: '留空保留已保存地址', removeTitle: '移除采集代理？',
        removeHint: '将从本次配置草稿移除此代理，保存后生效；不会删除账号原有业务代理。',
        rotateHint: '同账号/模型未获符合指定长度的票据才按列表切换下一条并重试；成功后停止，临近过期再采集。',
        fixedHint: '未获合格票时仍通过指定代理重试；成功后停止。不会切换到其他代理。',
        ipHint: '切换代理不等于保证换出口 IP。单条代理能否轮换出口由服务商决定；业务仍使用账号原代理。',
        targetLength: '合格票据长度（字节）', attempts: '每账号模型最多尝试次数', retryInterval: '失败重试间隔（秒）', interval: '自动轮次结束等待（秒）',
        invalidForm: '请填写代理名称和新增地址；次数为正整数，合格长度 6–8192 字节，重试间隔 1–30 秒，自动轮次间隔 6–3600 秒。开启至少需一条代理。',
        versionMismatch: '前后端版本不匹配，请确认全部实例已更新后重新加载。',
        title: '票据采集（可选，默认长度 292）',
        description: '默认关闭。开启后按模型列表为独立 OpenAI OAuth 账号采集票据；关闭后不采集、不注入，保留原转发。',
        warning: '按指定长度及原有前缀等条件接受票据，长度不代表模型质量。开启后有效票覆盖回合状态，缺票暂停对应模型，账号总开关不变。缓存 1 小时、提前 10 分钟续采，不监控业务 312。最多 4 路并发、单次 25 秒；手动批次按填写次数执行，成功即停，自动采集仍有每轮 30 秒预算。认证、限流、取消可提前结束。保存会使旧票退出使用，关闭恢复原转发。',
        proxy: '采集代理地址（仅采集使用）',
      ...
      features: {
        siteBillingMode: { title: '站点购买方式', both: '充值和订阅', recharge: '仅充值', subscription: '仅订阅' },
        team: {
