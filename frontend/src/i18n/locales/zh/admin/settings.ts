...
// Settings
    settings: {
      codexTicket: {
        models: '采集模型', modelsHint: '每行一个完整模型 ID · 最多 100 个 · 自动去重',
        signalLength: '降智信号（字节）', signalHint: '0 关闭 · 命中仅提示，不改判定',
        ...
        removeHint: '保存后移除采集代理，不影响业务代理。',
        rotateHint: '未获票 → 下一代理重试；获票 → 停止，临期续采。',
        fixedHint: '未获票 → 原代理重试；获票 → 停止。',
        ipHint: '换代理 ≠ 换出口 IP；业务代理不变。',
        targetLength: '合格长度（字节）', attempts: '每号 / 模型尝试上限', retryInterval: '重试间隔（秒）', interval: '自动轮后等待（秒）',
        ...
        title: '票据采集',
        description: '独立 OpenAI OAuth · 默认关闭',
        warning: '保存后旧票失效；开启时缺票模型暂停，账号总开关不变。',
        cacheTime: '缓存时间', renewEarly: '提前续采', workers: '并发上限',
        rules: '按模型列表采集与检查，账号须支持该模型。长度和前缀共同校验；长度不是质量结论。有效票覆盖回合状态，关闭恢复原转发。',
        limits: '单次 25s，自动每轮最多 30s；手动按尝试上限执行，成功即停。认证、限流或取消可提前结束。不监控业务 312。',
        proxy: '采集代理地址',
        configured: '已配置 · 留空保留 · 密码不回显',
        notConfigured: 'HTTP(S) / SOCKS5(h) · 凭据加密保存',
        clearProxy: '清除已保存代理（需关闭功能或填写新代理）',
        save: '保存票据设置', saved: '票据设置已保存'
      },
      title: '系统设置',
