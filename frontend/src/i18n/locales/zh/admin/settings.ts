...
// Settings
    settings: {
      codexTicket: {
        ...
        signalLength: '降智信号（字节）', signalHint: '0 关闭 · 命中时按账号策略处理',
        ...
        invalidForm: '请检查已选配置：模型不能为空，长度和时间须在范围内，次数为0或正整数；异常长度不能等于合格长度，提前续采须小于缓存时间。',
        ...
        limits: '单次25s，自动每轮最多30s；按账号尝试上限执行，成功即停。认证、限流或取消可提前结束。',
        proxy: '采集代理地址',
