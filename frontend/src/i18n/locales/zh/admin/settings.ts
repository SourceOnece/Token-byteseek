...
// Settings
    settings: {
      codexTicket: {
        ...
        rotateHint: '同账号/模型未获合格 292 才按列表切换下一条并重试；成功后停止，临近过期再采集。',
        fixedHint: '未获合格 292 时仍通过指定代理重试；成功后停止。不会切换到其他代理。',
        ipHint: '切换代理不等于保证换出口 IP。单条代理能否轮换出口由服务商决定；业务仍使用账号原代理。',
        attempts: '每轮最多尝试次数（1–10）', retryInterval: '失败重试间隔（秒）', interval: '轮次结束等待（秒）',
        invalidForm: '请填写代理名称和新增代理地址；尝试次数为 1–10，重试间隔 1–30 秒，轮次间隔 6–3600 秒。开启时至少需要一条代理。',
        versionMismatch: '前后端版本不匹配，请确认全部实例已更新后重新加载。',
        title: '292 打票（可选）',
        description: '默认关闭。开启后为可调度的独立 OpenAI OAuth 账号采集 Astra/Sol 票据；关闭后不采集、不注入，保留原转发。',
        warning: '开启后沿快照逻辑：有效 292 覆盖回合状态，缺票暂不调度该账号的对应模型，账号总开关不变。票据缓存 1 小时，剩余 10 分钟续采；不监控业务 312 触发提前重采。探测消耗额度，最多 4 路并发、每轮 30 秒、单次 25 秒；限流等错误会提前停止。未配置轮次间隔时默认 6 秒。此功能不保证满血或消除 overloaded；关闭恢复原转发，保存后多实例约 2 秒生效。',
        proxy: '采集代理地址（仅采集使用）',
