...
// Settings
    settings: {
      codexTicket: {
        models: 'Collection models', modelsHint: 'One exact model ID per line · Max 100 · Duplicates merged',
        signalLength: 'Signal length (bytes)', signalHint: '0 = off · Matches follow the account policy',
        ...
        removeHint: 'Removed on save. Production proxies are unchanged.',
        rotateHint: 'Miss → next proxy; acquired → stop, renew near expiry.',
        fixedHint: 'Miss → same proxy; acquired → stop.',
        ipHint: 'New proxy ≠ new exit IP. Production proxies are unchanged.',
        targetLength: 'Eligible ticket length (bytes)', attempts: 'Max attempts per account/model', retryInterval: 'Retry delay (seconds)', interval: 'Wait after automatic cycle (seconds)',
        invalidForm: 'Check selected settings: models are required; lengths and times must be within range; attempts allow 0. Signal and target lengths must differ; renewal must precede expiry.',
        versionMismatch: 'Frontend/backend version mismatch. Update all instances and reload.',
        title: 'Ticket collection',
        description: 'Standalone OpenAI OAuth · Off by default',
        warning: 'Saving invalidates old tickets. When enabled, missing tickets pause the model, not the account switch.',
        cacheTime: 'Cache lifetime', renewEarly: 'Early renewal', workers: 'Max workers',
        rules: 'Listed models are collected and checked; accounts must support them. Length and prefix are validated together; length is not quality. Valid tickets replace turn state. Disabling restores normal forwarding.',
        limits: '25s per attempt; 30s per automatic cycle. Account attempt limits apply; stop on success. Auth, rate limits or cancellation may end early.',
        proxy: 'Collection proxy URL',
        configured: 'Configured · Blank keeps address · Password hidden',
        notConfigured: 'HTTP(S) / SOCKS5(h) · Credentials encrypted at rest',
        clearProxy: 'Clear saved proxy (disable tickets or provide a replacement)',
        save: 'Save ticket settings', saved: 'Ticket settings saved'
      },
      ...
      features: {
        siteBillingMode: { title: 'Site Purchase Options', both: 'Recharge and subscription', recharge: 'Recharge only', subscription: 'Subscription only' },
        team: {
