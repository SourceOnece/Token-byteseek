...
// Accounts
    accounts: {
      // 采集诊断只翻译固定枚举，不能直接展示上游错误原文。
      ticketDiagnostic: {
        "previousAttempt": "Previous executed attempt; no new upstream request was sent",
        "budget": "Attempt limit {seconds}s",
        "elapsed": "Elapsed {seconds}s",
        "providerHTTP": "Proxy provider HTTP {status}",
        "currentPause": "Currently paused: {reason}",
        "resumeAt": "Earliest resume: {time}",
        "phases": {
          "eligibility": "Eligibility check",
          "queue": "Waiting for concurrency",
          "proxy": "Acquiring collection proxy",
          "harvest": "Candidate collection",
          "verify": "Business proxy verification",
          "publish": "Recheck and save"
        },
        "network": {
          "timeout": "Request timed out",
          "dns": "DNS resolution failed",
          "tls": "TLS certificate verification failed",
          "connection": "Network/proxy connection failed"
        },
      "reasons": {
        "proxy_timeout": "Proxy provider timed out (separate 8s limit)",
        "transport_timeout": "Connection, TLS handshake or response-header timeout",
        "concurrency_config": "Shared slot TTL is too short; increase gateway.concurrency_slot_ttl_minutes or lower the collection timeout",
          "network": "Network or proxy connection failed",
          "upstream": "Upstream returned an error",
          "invalid_ticket": "Ticket length or format rejected",
          "credential": "Usable credentials unavailable",
          "storage": "Database or ticket cache operation failed",
          "cancelled": "Task cancelled; legacy record has no specific reason",
          "proxy_config": "Collection proxy configuration or decryption failed",
          "proxy_provider": "Proxy provider failed or returned an invalid proxy",
          "cooldown": "Consecutive collection failures; waiting for cooldown",
          "ineligible": "Ineligible; legacy record has no specific reason",
          "account_changed": "Credentials or ticket settings changed",
          "concurrency_busy": "Waiting for a slot; no upstream request sent",
          "backoff": "Waiting for upstream retry window",
          "business_proxy": "No usable business proxy bound",
          "incomplete_response": "Missing successful completion or response model",
          "model_mismatch": "Response model differs from requested model",
          "length_signal": "Configured degradation length detected",
          "timeout": "Attempt request timed out",
          "round_timeout": "Automatic round budget exhausted",
          "task_timeout": "Parent task deadline exceeded",
          "client_disconnected": "Collection connection closed or stopped",
          "lease_lost": "Collection lease lost; remaining tasks stopped",
          "config_unavailable": "Ticket configuration unavailable; paused for safety",
          "service_stopped": "Service stopped or restarted",
          "config_disabled": "Ticket collection is switched off",
          "account_missing": "Account no longer exists",
          "unsupported_account": "Account type cannot collect tickets",
          "account_inactive": "Account disabled or in error state",
          "account_expired": "Account expired with auto-pause enabled",
          "account_overloaded": "Account is in upstream overload backoff",
          "account_rate_limited": "Account is in upstream rate-limit backoff",
          "account_cooldown": "Account is temporarily unavailable",
          "model_unsupported": "Account does not support this model",
          "attempt_limit": "Configured attempt limit reached",
          "unknown": "No recognized reason recorded"
        }
      },
      ticketWorkbench: {
        ...
        fields: { attempt_timeout_seconds: 'Attempt timeout (seconds)', target_length: 'Eligible length (bytes)', degraded_signal_length: 'Length signal (0 = off)', max_attempts: 'Attempt limit (0 = unlimited)', concurrency: 'Account collection concurrency (1–4)', cache_minutes: 'Cache lifetime (minutes)', refresh_before_minutes: 'Renew early (minutes)', retry_interval_seconds: 'Retry interval (seconds)', probe_interval_seconds: 'Automatic check interval (seconds)', failure_threshold: 'Consecutive failures (0 = no cooldown)', cooldown_seconds: 'Retry after cooldown (seconds)' },
        timeoutHint: '5–300s; default 25. Shared by proxy acquisition, collection and verification. Longer limits hold concurrency longer.',
      ...
      ticketPolicy: {
        ...
        saved: 'Ticket settings saved', guardRisk: 'Recovery revokes the observed ticket. This model may be unavailable until renewed. Answers are never replayed.',
        source: { account: 'Account proxy', gateway: 'Gateway proxy' },
        modes: { on: 'On', off: 'Off' },
        proxyActions: { custom: 'Set account proxy', inherit: 'Use gateway proxy' },
        guards: { off: 'Business watchdog off', observe: 'Length scheduling · No auto recollect', recover_length: 'Length signal → Recollect', recover_model: 'Model mismatch → Recollect', recover: 'Either signal → Recollect' },
      ...
      ticketCollect: {
        ...
        progress: 'Accounts × models', live: 'Latest 50 attempts', runStatus: 'Batch status',
        historyHint: 'Closing/disconnecting stops remaining work; tickets and logs are kept. Full logs are in History.',
      ...
      tickets: {
        lengthLabel: 'Actual {actual} bytes, target {target} bytes',
        degradedSignal: 'Degradation signal', openDetails: 'View {model} collection details',
        source: { manual: 'Manual', auto: 'Automatic' }, latestAt: 'Completed at', currentTicket: 'Current ticket',
        retryAfter: 'Harvest backoff until: {time}',
        attempt: '{proxy} · Attempt {count}', noHeader: 'No ticket header', badPrefix: 'Invalid prefix', completedNoTicket: 'Completed · No eligible ticket',
      ...
      quality: {
        ...
        rule: { full: 'Enable scheduling', degraded: 'Disable scheduling', failed: 'Keep scheduling' },
        protocol: 'API upstream protocol', actualProtocol: 'Tested protocol', protocolDefault: 'Account setting',
        poolRate: 'Pool full rate',
        timeout: 'Timeout per account (s)', dashboard: 'Codex pool tests', dashboardHint: 'Latest result per account; untested accounts are excluded from the rate.',
        detectedEmail: 'Account email', emailClaimHint: 'Token claim for display, not validation. RT/codes resolve email after exchange.',
        email: 'Account email', emailUnavailable: 'Email unavailable', viewAnswer: 'Test answer',
        ...
        schedule: {
          ...
          deleteMessage: 'Permanently delete this plan and history; cancel queued work. Active work stops on detection; sent requests may still use quota. No undo.',
          deletePreserved: 'Accounts, latest results and applied scheduling are kept.',
          periodic: 'Periodic checks', queued: 'Queued', stop: 'Stop run and pause plan', manualRun: 'Manual run', scheduledRun: 'Scheduled run',
          title: 'Codex Scheduled Tests', description: 'Server-side · Continues after closing · Per-run history',
          timingHint: 'First run waits one interval; later intervals start after completion. No overlap.',
          ...
          selectAll: 'Select eligible accounts', fixedSelection: 'Max 500 · Fixed list, excluding future accounts, shadows and Agent Identity',
          enabled: 'Enabled', paused: 'Paused', enable: 'Enable tests', pause: 'Pause tests', runNow: 'Run now', confirm: 'I accept recurring quota use and the scheduling rules above. Disabled, expiry and quota restrictions still apply.',
        ...
        warning: '{count} selected · {seconds}s each · Uses upstream quota',
        disclaimer: 'OpenAI OAuth / API Key only; other platforms, shadows and Agent Identity are skipped. Labels are keyword results, not capability ratings. Unselected accounts are unchanged.',
        model: 'Test model', effort: 'Reasoning effort', effortDefault: 'Upstream default (omit)', concurrency: 'Concurrency',
        prompt: 'Test prompt', keyword: 'Match keyword', keywordHint: 'Match only the complete visible answer, case-sensitive. No regex; prompt and reasoning summaries excluded.',
        rememberHint: 'Settings are saved per admin in this browser (prompt/keyword in plain text). Clearing browser data removes them. Selection and confirmation are not saved.',
        confirm: 'I accept the scheduling rules above. Disabled, expiry and quota restrictions still apply.',
        ...
        noAnswer: 'No answer', schedulingOn: 'Scheduling on · Other limits apply', schedulingOff: 'Scheduling off', schedulingUnchanged: 'Scheduling unchanged',
        start: 'Test & apply', stop: 'Stop tests', progress: 'Testing {done} / {total}', completed: 'Finished {done} / {total}', stopped: 'Stopped {done} / {total} · Applied scheduling kept',
      ...
      opencodeGo: {
        ...
        protocolRules: {
          invalid: 'Use up to 64 rules with nonempty model IDs and trailing * wildcards only.',
      ...
      cnProviders: {
        accountMode: {
          zen: 'Zen',
          zenDesc: 'OpenCode pay-as-you-go gateway.',
          go: 'GO',
          goDesc: 'OpenCode subscription quota with 5-hour, weekly and monthly windows.',
          title: 'Account Type',
          payg: 'Pay-as-you-go',
          paygDesc: 'Consumes account balance, billed per token. Auto-cools down on low balance and recovers after top-up.',
          minimaxPaygDesc: 'MiniMax pay-as-you-go key. Balance lookup is unavailable.',
          coding: 'Coding Plan',
