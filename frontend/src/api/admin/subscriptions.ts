...
export interface BulkAssignSubscriptionResult {
  ...
  errors: string[]
  statuses?: Record<string, 'active' | 'queued' | 'failed'>
}

// 参数和操作键由确认弹窗冻结，网络错误后不得自动生成新的操作键。
export async function bulkAction(request: SubscriptionBulkActionRequest, idempotencyKey: string): Promise<SubscriptionBulkActionResult> {
  const { data } = await apiClient.post<SubscriptionBulkActionResult>('/admin/subscriptions/bulk-action', request, {
