  ...
  >
    <form id="bulk-edit-keys-form" class="space-y-5" @submit.prevent="submit">
      ...
      <fieldset :disabled="submitting" class="space-y-5">
        <div class="space-y-2">
          <label class="flex items-center gap-2 text-sm font-medium" :title="hasCompositeKeys ? t('keys.bulkEdit.compositeGroupLocked') : undefined">
            <input v-model="enabled.group_id" type="checkbox" class="checkbox" data-test="enable-group" :disabled="hasCompositeKeys" />
            {{ t('keys.groupLabel') }}
          </label>
          <Select
...
type SelectedKey = Pick<ApiKey, 'id' | 'name'> & Partial<Pick<ApiKey, 'is_composite'>>
...
const hasCompositeKeys = computed(() => pendingKeys.value.some(key => key.is_composite))
...
const validationError = computed(() => {
  if (enabled.group_id && hasCompositeKeys.value) return t('keys.bulkEdit.compositeGroupLocked')
...
watch(() => props.show, (show) => {
  if (!show) return
  // 不携带明文 Key；复合 Key 不可被普通单分组批量修改隐式转换。
  pendingKeys.value = props.selectedKeys.map(({ id, name, is_composite }) => ({ id, name, is_composite }))
  failures.value = []
