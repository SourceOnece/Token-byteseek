...
describe('BulkEditKeysModal', () => {
  it('选择复合 Key 不允许批量改单分组，仍可批量改状态', async () => {
    const wrapper = mountModal()
    await wrapper.setProps({ show: false })
    await wrapper.setProps({ show: true, selectedKeys: [{ id: 1, name: 'Composite', is_composite: true }] })
    ...
    await flushPromises()
    expect(bulkUpdate).toHaveBeenCalledWith([1], { status: 'active' })
    wrapper.unmount()
  })
