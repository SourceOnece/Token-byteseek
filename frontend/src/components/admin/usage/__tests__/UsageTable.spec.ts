...
vi.mock('@/composables/useBalanceDisplay', () => ({
  useBalanceDisplay: () => ({
    ...
    usdUnitSymbol: '$',
    formatBalanceAmount: (value: number | null | undefined, options?: { fractionDigits?: number }) => `$${(value ?? 0).toFixed(options?.fractionDigits ?? 6)}`,
    formatUsdAmount: (value: number | null | undefined, options?: { fractionDigits?: number }) => `$${(value ?? 0).toFixed(options?.fractionDigits ?? 6)}`,
  }),
}))
