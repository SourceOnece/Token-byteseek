  ...
  >
    <form id="bulk-subscription-action-form" class="space-y-4" novalidate @submit.prevent="submit">
      <div>
        ...
        <ul class="max-h-48 divide-y divide-gray-100 overflow-y-auto rounded-none border-2 border-[var(--bh-ink)] dark:divide-dark-700 dark:border-dark-600">
          <li v-for="subscription in targets" :key="subscription.id" class="px-3 py-2 text-sm">
            ...
            <div class="break-words text-xs text-gray-500 dark:text-gray-400">
              {{ subscription.group || t('admin.subscriptions.bulk.groupFallback', { id: subscription.planId }) }}
      ...
      <fieldset :disabled="parametersLocked" class="space-y-3 disabled:opacity-70">
        <div v-if="currentAction === 'extend'">
          <label for="bulk-subscription-days" class="input-label">
            {{ t('admin.subscriptions.bulk.adjustDays') }}
          </label>
          <input
    ...
    <template #footer>
      <div class="flex flex-wrap justify-end gap-3">
        ...
        >
          <Icon :name="submitting ? 'refresh' : currentAction === 'revoke' ? 'trash' : 'check'" size="sm" class="mr-2" :class="{ 'animate-spin': submitting }" />
...
const targets = ref<{ id: number; email?: string; group?: string; planId: number }[]>([])
...
async function submit() {
  ...
  try {
    ...
    // 超时或服务端错误不等于未写入；结果未知时只重试原参数和操作键。
