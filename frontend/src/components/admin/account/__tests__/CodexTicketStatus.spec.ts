...
describe('CodexTicketStatus', () => {
  it.each(['new', 'reused'] as const)('悬停与点击详情都显示IP来源：%s，读取失败隐藏旧标签', async usage => {
    ...
    expect(w.get('[title]').attributes('title')).toContain('ticketWorkbench.ipUsage.' + usage)
    await w.get('[data-testid="ticket-model-detail"]').trigger('click')
    expect(w.get('[data-testid="ticket-ip-usage"]').text()).toContain('ticketWorkbench.ipUsage.' + usage)
    await w.setProps({ failed: true })
    expect(w.find('[data-testid="ticket-ip-usage"]').exists()).toBe(false)
    expect(w.get('[title]').attributes('title')).not.toContain('ticketWorkbench.ipUsage.')
    w.unmount()
  })
