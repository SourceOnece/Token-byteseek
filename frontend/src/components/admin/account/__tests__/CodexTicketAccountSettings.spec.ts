...
describe('CodexTicketAccountSettings', () => {
  it('超时共同值回填，不同值留空；勾选后仅提交超时，取消勾选不提交', async () => {
    get.mockImplementation(async id=>({...account(), rules:{...rules(), attempt_timeout_seconds:id===1?120:90}}))
    ...
    const input=w.get('[data-testid="ticket-rule-attempt_timeout_seconds"]')
    expect((input.element as HTMLInputElement).value).toBe('')
    expect(input.attributes('disabled')).toBeDefined()
    await w.get('[data-testid="ticket-edit-attempt_timeout_seconds"]').setValue(true)
    await input.setValue('120')
    expect((w.vm as unknown as {patch:()=>unknown}).patch()).toEqual({mode:'on',rules:{attempt_timeout_seconds:120}})
    await w.get('[data-testid="ticket-edit-attempt_timeout_seconds"]').setValue(false)
    expect((w.vm as unknown as {patch:()=>unknown}).patch()).toEqual({mode:'on'})
    w.unmount()
    get.mockResolvedValue({...account(),rules:{...rules(),attempt_timeout_seconds:90}})
    const common=mount(CodexTicketAccountSettings,{props:{ids:[1,2],bulk:true}});await flushPromises()
    expect((common.get('[data-testid="ticket-rule-attempt_timeout_seconds"]').element as HTMLInputElement).value).toBe('90')
    common.unmount()
  })
  it('模板超时随原保存提交，边界校验保留；旧后端不能静默忽略新超时', async () => {
    defaults.mockResolvedValue({...account(),rules:{...rules(),attempt_timeout_seconds:25}})
    const w=mount(CodexTicketAccountSettings,{props:{templateMode:true}});await flushPromises()
    const input=w.get('[data-testid="ticket-rule-attempt_timeout_seconds"]')
    await input.setValue('301')
    expect(()=>(w.vm as unknown as {patch:()=>unknown}).patch()).toThrow()
    await input.setValue('120');await save(w)
    expect(updateDefaults.mock.calls[0][0].rules.attempt_timeout_seconds).toBe(120)
    w.unmount()
    get.mockResolvedValue(account())
    const old=mount(CodexTicketAccountSettings,{props:{ids:[1]}});await flushPromises()
    await old.get('[data-testid="ticket-rule-attempt_timeout_seconds"]').setValue('120')
    expect(()=>(old.vm as unknown as {patch:()=>unknown}).patch()).toThrow('versionMismatch')
    old.unmount()
  })
