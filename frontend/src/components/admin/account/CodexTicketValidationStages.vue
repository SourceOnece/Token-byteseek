<template>
  <div v-if="stages?.length" class="grid gap-4 sm:grid-cols-2" data-testid="ticket-validation-stages">
    <section v-for="stage in stages" :key="stage.name" class="min-w-0 space-y-3 border-2 border-[color:var(--bh-ink)] bg-[var(--bh-surface)] p-4" style="box-shadow:var(--bh-shadow-sm)">
      ...
      <p v-if="stage.reason" class="text-sm font-bold" :class="stage.reason === 'length_signal' || stage.reason === 'model_mismatch' ? 'text-bh-red dark:text-red-400' : 'text-yellow-800 dark:text-bh-yellow'">{{ ticketReasonText(t, stage.reason) }}</p>
    </section>
  </div>
</template>
<script setup lang="ts">
import { ticketReasonText } from '@/utils/codexTicketDiagnostic'
