<template>
  <div class="space-y-4" data-testid="ticket-proxy-editor">
    <div class="grid gap-4 sm:grid-cols-2">
      <div><label :for="uid + '-mode'" class="input-label">{{ t('admin.accounts.ticketWorkbench.proxyMode') }}</label><Select :id="uid + '-mode'" v-model="mode" :options="modeOptions" :placeholder="' '" :disabled="locked || testing" /></div>
    ...
    <template v-if="mode && mode !== 'inherit'">
      ...
      <template v-else>
        <div v-for="(row, index) in rows" :key="row.id" class="space-y-2 border-l-4 border-bh-blue pl-3">
          ...
          <Select v-model="row.source" :options="[{value:'manual',label:zh?'手动填写':'Enter manually'},{value:'managed',label:zh?'从代理管理选择':'Choose managed proxy'}]" :disabled="locked || testing" @update:model-value="row.url=''; row.configured=false" />
          <Select v-if="row.source==='managed'" v-model="row.managed_proxy_id" :options="managedOptions" searchable :disabled="locked || testing" :placeholder="' '" />
          <input v-else v-model="row.url" type="password" autocomplete="new-password" class="input w-full font-mono" :aria-label="t('admin.accounts.ticketPolicy.proxy')" :disabled="locked || testing" :placeholder="row.configured ? t('admin.settings.codexTicket.keepAddress') : 'http://user:password@proxy.example:8080'" data-testid="ticket-proxy-url" />
        </div>
...
const props = defineProps<{ value?: TicketProxyPolicy; locked?: boolean; accountId?: number; allowInherit?: boolean; inherited?: boolean; testDisabled?: boolean; blank?: boolean; templateSource?: boolean }>()
const { t, locale } = useI18n(), uid = useId()
const zh = computed(()=>!locale?.value?.startsWith('en'))
const managed = ref<{id:number;name:string;host:string;port:number}[]>([])
const managedOptions = computed(()=>managed.value.map(p=>({value:p.id,label:p.name+' · '+p.host+':'+p.port})))
async function loadManaged() { try {managed.value=await loadManagedProxies()} catch {error.value=t('common.error')} }
...
function add() { const bytes = crypto.getRandomValues(new Uint8Array(16)); bytes[6] = (bytes[6] & 15) | 64; bytes[8] = (bytes[8] & 63) | 128; const h = Array.from(bytes, n => n.toString(16).padStart(2, '0')).join(''); const id = [h.slice(0, 8), h.slice(8, 12), h.slice(12, 16), h.slice(16, 20), h.slice(20)].join('-'); rows.value.push({ id, name: `Proxy ${rows.value.length + 1}`, url: '', configured: false, source: 'manual' }); if (!fixedID.value) fixedID.value = id }
...
function patch(): TicketProxyPatch {
  if (!mode.value) throw new Error(t('admin.accounts.ticketPolicy.proxyRequired'))
