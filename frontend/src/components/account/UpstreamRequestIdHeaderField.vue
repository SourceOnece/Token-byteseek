...
<template>
  <div>
          ...
          <ul class="mt-1 space-y-1.5">
            <li v-for="item in examples" :key="item.label">
              <div class="flex items-center justify-between gap-3">
                ...
                <code class="select-all rounded-none border border-white/30 bg-white/10 px-1.5 py-0.5 font-mono text-[11px] text-white">{{ item.header }}</code>
              </div>
    ...
    <input
      ...
      type="text"
      class="input"
      :aria-label="t('admin.accounts.upstreamRequestIdHeader')"
      maxlength="64"
