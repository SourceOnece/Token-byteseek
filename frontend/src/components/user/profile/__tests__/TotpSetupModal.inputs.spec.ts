...
async function openCodeStep() {
  ...
  await flushPromises()
  await wrapper.get('.btn-primary').trigger('click')
  const inputs = wrapper.findAll('[data-testid="totp-digit-input"]')
...
describe('TOTP setup input synchronization', () => {
  ...
  it('displays retained digits when returning from the QR-code step', async () => {
    ...
    await wrapper.get('.btn-primary').trigger('click')
    expect(wrapper.findAll('[data-testid="totp-digit-input"]').map(input => (input.element as HTMLInputElement).value)).toEqual(['1', '2', '3', '4', '5', '6'])
    await wrapper.get('form').trigger('submit')
    await flushPromises()
