...
describe('PaymentProviderDialog payment guide', () => {
  ...
  it.each(['epay', 'usdt.trc20'])('serializes EasyPay upstream type %s and keeps local type naming', async (upstreamType) => {
    const provider = providerFactory({
  ...
  it.each([['alipay_hk', 'hkpay'], ['usdt.trc20', 'usdt.trc20'], ['usdt_trc20', 'usdt/trc20']])('rejects invalid EasyPay type %s to %s', async (type, upstreamType) => {
    const provider = providerFactory({
      provider_key: 'easypay',
      name: 'EasyPay',
      config: {
