...
import { purchaseLabelKey, type BillingModeSettings } from '@/utils/siteBillingMode'
...
export function resolveRouteDocumentTitle(
  ...
  billingSettings?: BillingModeSettings | null,
): string {
  ...
  const titleKey = route.name === 'PurchaseSubscription' && billingSettings
    ? purchaseLabelKey(billingSettings)
